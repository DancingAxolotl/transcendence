...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("TranscendenceMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("TranscendenceMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("TranscendenceMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("transcendence-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running TranscendenceMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
