...
void PrepareShutdown()
{
    ...
    RenameThread("transcendence-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "transcendence.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "transcendenced.pid"));
#endif
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in TRANSCENDENCE/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in TRANSCENDENCE/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in TRANSCENDENCE/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Transcendence specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:8051"));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i-Hell Freezes The Transcendence Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(_("")) + "\n" +
           "\n" +
...
{
    RenameThread("transcendence-loadblk");
...
/** Sanity checks
 *  Ensure that Transcendence is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Transcendence Core is shutting down."));
...
#endif
    // Make sure only a single Transcendence process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Transcendence Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Transcendence version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // Transcendence: load previous sessions sporks if we have them.
                ...
                // Transcendence: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Transcendence Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Transcendence Core to complete") << "\n";
        ...
        */
        ...
        bool fEnableZTranscendenceBackups = GetBoolArg("-backupztranscendence", true);
        pwalletMain->setZTranscendenceAutoBackups(fEnableZTranscendenceBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeTranscendenceAmount = 2;
...
//    }
//
//    nAnonymizeTranscendenceAmount = GetArg("-anonymizetranscendenceamount", 0);
//    if (nAnonymizeTranscendenceAmount > 999999) nAnonymizeTranscendenceAmount = 999999;
//    if (nAnonymizeTranscendenceAmount < 2) nAnonymizeTranscendenceAmount = 2;
    ...
    LogPrintf("Anonymize Transcendence Amount %d\n", nAnonymizeTranscendenceAmount);
    ...
    /* Denominations
       ...
       For example:
       1TRANSCENDENCE+1000 == (.1TRANSCENDENCE+100)*10
       10TRANSCENDENCE+10000 == (1TRANSCENDENCE+1000)*10
    */
